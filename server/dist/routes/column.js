...
router.get("/columns/:userEmail", validateToken_1.validateToken, async (req, res) => {
    try {
        const requestingUserEmail = req.user?.email;
        const { userEmail } = req.params;
        // Only allow if requesting user is admin or accessing their own data
        const requestingUser = await User_1.User.findOne({ email: requestingUserEmail });
        if (!requestingUser?.isAdmin && requestingUserEmail !== userEmail) {
            res.status(403).json({ error: "Access denied" });
            return;
        }
        console.log("Fetching columns for user:", userEmail);
        const columns = await Column_1.Column.find({ owner: userEmail }).sort({ order: 1 });
        res.status(200).json(columns);
    }
    catch (error) {
        res.status(500).json({ error: "Failed to get columns" });
...
router.put("/columns/:id", validateToken_1.validateToken, async (req, res) => {
    try {
        ...
        if (order) {
            ...
            if (typeof column1.order === "number" &&
