...
router.get("/columns", validateToken_1.validateToken, async (req, res) => {
  try {
    ...
    const columns = await Column_1.Column.find({ owner: userId }).sort({
      order: 1,
    });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const columns = await Column_1.Column.find({ owner: userEmail }).sort({
        order: 1,
      });
      res.status(200).json(columns);
    } catch (error) {
...
router.post("/column", validateToken_1.validateToken, async (req, res) => {
  try {
    ...
    const columnAmount = await Column_1.Column.countDocuments({
      owner: userId,
    });
...
router.put("/column/:id", validateToken_1.validateToken, async (req, res) => {
  try {
    ...
    const updatedColumn = await Column_1.Column.findByIdAndUpdate(
      id,
      { name },
      { new: true },
    );
