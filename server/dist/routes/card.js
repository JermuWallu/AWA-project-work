...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const cards = await Card_1.Card.find({ columnID: columnId }).sort({
        order: 1,
      });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const updatedCard = await Card_1.Card.findByIdAndUpdate(
        columnId,
        { column: newColumnId },
        { new: true },
      );
...
router.put("/card/:id", validateToken_1.validateToken, async (req, res) => {
  try {
    ...
    const updatedCard = await Card_1.Card.findByIdAndUpdate(id, updateData, {
      new: true,
    });
    if (!updatedCard) {
